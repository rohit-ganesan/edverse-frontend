...
export function StatsCard({
  ...
  return (
    <RadixCard
      className={`p-0 shadow-lg hover:shadow-xl border-0 bg-white dark:bg-gray-800 overflow-hidden transition-all duration-300 hover:scale-[1.02] ${className || ''}`}
    >
      <Box className="p-5">
        <Flex align="center" gap="4">
          {/* Enhanced icon container */}
          <Box
            className={`w-14 h-14 ${iconBgColor || 'bg-blue-100'} dark:bg-gray-700 rounded-xl flex items-center justify-center shadow-sm ring-1 ring-gray-100`}
          >
            <IconComponent
              className={`w-7 h-7 ${iconColor || 'text-blue-600'} dark:text-gray-200`}
            />
          </Box>
